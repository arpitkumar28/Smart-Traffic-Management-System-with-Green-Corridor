...
export default function LiveMap() {
    ...
    const initMap = async () => {
      try {
        ...
        setMapComponents({
          ...
          Circle: RLModule.Circle,
          Tooltip: RLModule.Tooltip,
        });
      } catch (err) {
  ...
  const { L, MapContainer, TileLayer, Marker, Popup, Polyline, Circle, Tooltip } = MapComponents;

  const defaultCenter: [number, number] = [28.6139, 77.209];
  const mapCenter: [number, number] = defaultCenter;

  const getTrafficColor = (load: number) => {
    if (load > 80) return "#ff4d4d"; // Heavy (Emergency Red)
    if (load > 40) return "#ffc857"; // Medium (Warning Yellow)
    return "#00ff9d"; // Low (Success Green)
  };

  const getSignalStatusColor = (status: string) => {
    const s = status?.toLowerCase() || "";
    if (s === "red") return "#ff4d4d";
    if (s === "yellow") return "#ffc857";
    if (s === "priority" || s === "green_corridor") return "#00e5ff"; 
    return "#00ff9d"; 
  };

  return (
    <div className="h-[calc(100vh-10rem)] w-full overflow-hidden rounded-xl border border-white/10 bg-[#030712] shadow-2xl relative">
      <MapContainer 
        center={mapCenter} 
        zoom={14} 
      ...
      >
        ...
        {signals.map((signal) => {
          ...
          const isCongested = signal.traffic_load > 80;
            ...
            html: `
              <div class="relative flex items-center justify-center">
                <div style="
                  ...
                  ${isPriority ? 'animation: pulse 1.5s infinite;' : ''}
                ">
                ...
                <div class="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap">
                   <span class="px-2 py-0.5 rounded text-[8px] font-black tracking-tighter shadow-sm border ${
                    ...
                    ${isPriority ? '🚨 PRIORITY' : isCongested ? '🔴 CONGESTED' : '🟢 NORMAL'}
                   </span>
                </div>
              </div>`,
            iconSize: [24, 24],
              ...
              >
                <Popup>
                  <div className="p-2 text-slate-900 min-w-[150px]">
                    <h3 className="font-bold border-b border-slate-200 pb-1 text-sm uppercase">{signal.name}</h3>
                    <div className="mt-2 space-y-1 text-xs font-medium">
                      ...
                      <p className="flex justify-between">
                        ...
                        <span className="font-bold" style={{ color: getTrafficColor(signal.traffic_load) }}>
                          {signal.traffic_load}% ({signal.traffic_load > 80 ? "Critical" : signal.traffic_load > 40 ? "Elevated" : "Optimal"})
                        </span>
                      </p>
                    </div>
                  </div>
            ...
            html: `
              <div class="relative ${isActive ? 'animate-bounce' : ''}">
                <div style="
                  width: 32px; 
                  height: 32px; 
                  background: ${isActive ? '#00e5ff' : '#ff4d4d'}; 
                  border: 2px solid white; 
                  border-radius: 6px; 
                  box-shadow: 0 0 20px ${isActive ? '#00e5ff' : '#ff4d4d'};
                  display: flex;
                  align-items: center;
                  justify-content: center;
                  color: white;
                ...
                ${isActive ? `
                  <div class="absolute -right-16 top-1/2 -translate-y-1/2 bg-cyan text-black px-2 py-0.5 rounded text-[8px] font-black whitespace-nowrap border border-white/20">
                    HOSPITAL 🏥
                  </div>
                ` : ''}
              </div>`,
      ...
      <div className="absolute top-6 right-6 z-[1000] flex flex-col gap-3">
          <div className="glass-panel p-3 min-w-[120px]">
              <p className="text-[9px] font-black text-white/40 uppercase tracking-widest">Network Load</p>
              <p className="text-xl font-black text-warning">64%</p>
          </div>
          <div className="glass-panel p-3 min-w-[120px]">
              <p className="text-[9px] font-black text-white/40 uppercase tracking-widest">Active Units</p>
              <p className="text-xl font-black text-primary">{emergencyVehicles.length}</p>
          </div>
          <div className="glass-panel p-3 min-w-[120px]">
              <p className="text-[9px] font-black text-white/40 uppercase tracking-widest">Sync Score</p>
      ...
      {corridorData && (
          <div className="absolute top-6 left-6 z-[1000] max-w-xs animate-in slide-in-from-left duration-500">
              <div className="rounded-xl border border-cyan/40 bg-[#030712]/95 p-4 shadow-2xl backdrop-blur-md">
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-3">
                      <div>
                          ...
                          <p className="text-lg font-black text-[#00ff9d]">-{corridorData.timeSaved}m</p>
                      </div>
                      <div>
                          <p className="text-[9px] text-white/40 uppercase font-bold">Signals Sync</p>
                          <p className="text-lg font-black text-[#00e5ff]">{corridorData.signalsSynced}/18</p>
                      </div>
                  </div>
                  <div className="mt-3 bg-white/5 rounded p-2 border border-white/5">
                      <p className="text-[9px] text-white/40 uppercase font-bold mb-1">Destination ETA</p>
                      <p className="text-xs font-bold text-white/80">7 min | {corridorData.destination}</p>
                  </div>
              </div>
          </div>
      )}
      ...
      {/* Map Legend */}
      <div className="absolute bottom-6 right-6 z-[1000] rounded-lg border border-white/10 bg-[#030712]/90 p-4 text-[10px] text-white shadow-xl backdrop-blur-md">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-[#00ff9d]" /> <span>Green (Optimal)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-[#ffc857]" /> <span>Yellow (Elevated)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-[#ff4d4d]" /> <span>Red (Congested)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-[#00e5ff] shadow-[0_0_8px_#00e5ff]" /> <span>Priority Corridor</span>
          </div>
